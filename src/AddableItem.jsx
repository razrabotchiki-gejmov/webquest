...
const AddableItem = ({
  ...
  setQuestionType,
  setActiveQuestion,
  setInteractionMessage, // Добавлено
  setInteractionItemName, // Добавлено
  setInteractionItemDescription, // Добавлено
}) => {
  const ref = useRef();
  ...
  useFrame(() => {
    ...
    if (intersects.length > 0 && intersects[0].distance < threshold && keys['KeyE']) {
      ...
      if (setInteractionMessage && setInteractionItemName && setInteractionItemDescription) {
        setInteractionMessage(true);
        setInteractionItemName(name); // Установка названия предмета
        setInteractionItemDescription(description); // Установка описания предмета
      }
    }
